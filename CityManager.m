...
- (nullable NSString *)getProvinceNameWithCode:(NSString *)code {
    ...
    if ([prefix isEqualToString:@"81"]) return @"中国香港";
    if ([prefix isEqualToString:@"82"]) return @"中国澳门";
    if ([prefix isEqualToString:@"71"]) return @"中国台湾";
    
    return _provinceMap[prefix] ?: nil;
}

- (nullable NSString *)getCityNameWithCode:(NSString *)code {
    if (!code || code.length < 4) return nil;
    
    NSString *provincePrefix = [code substringToIndex:2];
    
    if ([@[@"81", @"82", @"71"] containsObject:provincePrefix]) {
        return _cityMap[[code substringToIndex:4]] ?: nil;
    }
    
    
    if ([@[@"11", @"12", @"31", @"50"] containsObject:provincePrefix]) {
        return [self getDistrictNameWithCode:code]; 
    }
...
- (nullable NSString *)getDistrictNameWithCode:(NSString *)code {
    ...
    NSString *cityPrefix = [code substringToIndex:4];

    
    if (code.length >= 6) {
        NSString *districtCode = [code substringToIndex:6];
        NSString *result = _districtMap[districtCode];
        if (result) return result;
    }

    
    NSArray *candidates = [[_districtMap allKeys] filteredArrayUsingPredicate:
                           [NSPredicate predicateWithFormat:@"SELF BEGINSWITH %@", cityPrefix]];
    if (candidates.count > 0) {
        NSString *firstCode = candidates.firstObject;
        return _districtMap[firstCode];
    }
...
- (nullable NSString *)getStreetNameWithCode:(NSString *)code {
    ...
    NSString *districtPrefix = [code substringToIndex:6];

   
    if (code.length >= 9) {
        NSString *streetCode = [code substringToIndex:9];
        NSString *result = _streetMap[streetCode];
        if (result) return result;
    }

    
    NSArray *candidates = [[_streetMap allKeys] filteredArrayUsingPredicate:
                           [NSPredicate predicateWithFormat:@"SELF BEGINSWITH %@", districtPrefix]];
    if (candidates.count > 0) {
        NSString *firstCode = candidates.firstObject;
